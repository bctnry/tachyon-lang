...
// Create a new word object.  This function requires `val` to be static
// string (i.e. string literals or other strings that have a fixed location.)
TachyonDatum* Tachyon_New_WordFromStatic(char* val) {
	TachyonDatum* res = (TachyonDatum*)calloc(1, sizeof(TachyonDatum));
	res->type = TACHYON_WORD;
	res->v.strval = String_NewFromStatic(val);
	return res;
}

// Create a new word object.  This function takes the ownership from its
// caller. The caller should *NOT* try to free the underlying char array.
TachyonDatum* Tachyon_New_WordFromAllocated(char* val) {
	TachyonDatum* res = (TachyonDatum*)calloc(1, sizeof(TachyonDatum));
	res->type = TACHYON_WORD;
	res->v.strval = String_NewFromAllocated(val);
	return res;
}
...
void dump_env(TachyonEnvPage* env) {
	ArrayList_ForEach(env, dump_env_1);
}
...
void Tachyon_Exec(TachyonEnv* env, TachyonDatum* d) {
	...
	case TACHYON_WORD: {
		if (strcmp(d->v.strval->base, "[") == 0) {
			...
			TachyonDatum* a = Tachyon_Env_PopStack(env);
			TachyonDatum* b = Tachyon_Env_PopStack(env);
			char* ar = Tachyon_DatumToString(a);
			char* br = Tachyon_DatumToString(a);
			...
			char* ss = Tachyon_Aux_StringCopy(s->base);
			TachyonDatum *a = Tachyon_New_WordFromAllocated(ss);
			Tachyon_Env_PushStack(env, a);
			String_Dispose(s);
		} else {
...
int main() {
    ...
    Tachyon_Exec(env, Tachyon_New_WordFromStatic("'Factorial"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("["));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("'a"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("swap"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("#"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("'a"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("$"));
	Tachyon_Exec(env, Tachyon_New_Int(0));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("=="));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("["));
	Tachyon_Exec(env, Tachyon_New_Int(0));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("]"));
	...
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("'a"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("$"));
	Tachyon_Exec(env, Tachyon_New_Int(1));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("=="));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("["));
	Tachyon_Exec(env, Tachyon_New_Int(1));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("]"));
	...
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("'a"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("$"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("dup"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("dec"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("Factorial"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("*"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("]"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("if"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("]"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("if"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("]"));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("#"));
	Tachyon_Exec(env, Tachyon_New_Int(10));
	Tachyon_Exec(env, Tachyon_New_WordFromStatic("Factorial"));
	printf("------------------------\n");
	dump_stack(env);
